...
validate_backend_non_negative_integer_arg() {
	local name="$1"
	local value="$2"
	if [[ "$value" =~ ^[0-9]+$ ]]; then
		return 0
	...
	echo "build_agent_command: invalid ${name} (expected: non-negative integer)" >&2
	return 1
}
...
build_agent_command() {
	...
	prompt_file_arg="$(backend_shell_quote_arg "$prompt_file")"
	...
	if [[ -n "$effective_model" ]]; then
		model_flag=" --model $(backend_shell_quote_arg "$effective_model")"
	fi

	local max_turns_flag=""
	if [[ -n "$max_turns" ]] && [[ "$backend" == "claude" ]]; then
		validate_backend_non_negative_integer_arg "--max-turns" "$max_turns" || return 1
		max_turns_flag=" --max-turns ${max_turns}"
	fi

	local thinking_budget_flag=""
	if [[ -n "$thinking_budget" ]] && [[ "$backend" == "claude" ]]; then
		validate_backend_non_negative_integer_arg "--thinking-budget" "$thinking_budget" || return 1
		...
		if [[ -n "$claude_effort" ]]; then
			case "$claude_effort" in
				low | medium | high | xhigh | max)
					effort_flag=" --effort $(backend_shell_quote_arg "$claude_effort")"
					;;
				*)
	...
	local stream_file_arg
	local stderr_log_arg
	local formatter_arg
	stream_file_arg="$(backend_shell_quote_arg "$stream_file")"
	stderr_log_arg="$(backend_shell_quote_arg "$stderr_log")"
	formatter_arg="$(backend_shell_quote_arg "$formatter")"
	...
	case "$backend" in
		acp)
			...
			cmd="acpx --approve-all --cwd \"\${project_dir}\" claude exec \"\$(cat ${prompt_file_arg})\"${model_flag}"
			;;
		acp-codex)
			# ACP wraps codex via acpx — always interactive (visible terminal).
			cmd="acpx --approve-all --cwd \"\${project_dir}\" codex exec \"\$(cat ${prompt_file_arg})\"${model_flag}"
			;;
		acp-gemini)
			# ACP wraps gemini via acpx — always interactive (visible terminal).
			cmd="acpx --approve-all --cwd \"\${project_dir}\" gemini exec \"\$(cat ${prompt_file_arg})\"${model_flag}"
			;;
		gemini)
			if [[ -n "$interactive" ]]; then
				cmd="gemini \"\$(cat ${prompt_file_arg})\" --approval-mode yolo${model_flag}"
			else
				if [[ -n "$script_dir" ]] && [[ -x "$formatter" ]]; then
					cmd="gemini -p \"\$(cat ${prompt_file_arg})\" --approval-mode yolo --output-format stream-json${model_flag} 2>>${stderr_log_arg} | tee ${stream_file_arg} | ${formatter_arg}"
				else
					cmd="gemini -p \"\$(cat ${prompt_file_arg})\" --approval-mode yolo --output-format stream-json${model_flag} 2>>${stderr_log_arg} | tee ${stream_file_arg}"
				fi
			fi
			;;
		codex)
			...
			if [[ -n "$project_dir" ]]; then
				codex_workspace_flags=" --cd $(backend_shell_quote_arg "$project_dir") --add-dir $(backend_shell_quote_arg "${project_dir}/.git")"
			fi
