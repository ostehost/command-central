...
**Lane registry (primary, zero-config).** `commandCentral.laneRegistry.files` defaults to `["~/.config/openclaw/lanes.json", "~/.config/ghostty-launcher/tasks.json"]` — a transitional OpenClaw-namespace bridge plus the deprecated launcher compat path. Two file shapes are recognized: the launcher `{version, tasks: {...}}` registry and the transitional Work System lanes read-model/projection `{version: 1, kind: "work-system-lanes-projection", lanes: {<lane_ref.id>: <lane_ref_update>}, updated_at}` written by the launcher bridge in outbox mode (envelopes transform into AgentTask rows; `lane_ref.task` becomes the task id, `lane_kind`/`lane_kind_source`/status/session/worktree/updatedAt are preserved). Either shape ingests `lane-records-only`: only Work Registry-backed LaneRef records (records carrying `project_ref.id`) are admitted; launcher-era rows and projection envelopes without `project_ref` stay quarantined (logged count). Projection rows are never authoritative truth — a primary registry record with the same task id wins the merge regardless of file order. An explicit empty list reads nothing. Long-term primary source is the OpenClaw-native Work System plugin/API (`workSystem.lanes.list` + per-session `workSystem` projection from plugin session extensions), not a file path.
...
Schema: `{"version": 2, "tasks": {"<id>": <AgentTask>}}`. Versions 1 and 2 are both supported; default to 2 when creating. The lanes projection shape (`kind: "work-system-lanes-projection"`, version 1) is read-only bridge compatibility — never create it from Command Central.
