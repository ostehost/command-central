...
📦 **Since previous prerelease cut (rc85)**
  • `74de6e80` fix(agent-status): scope the idle-REPL cue to clean completions
  • `e4249179` fix(vsix-gate): pin a helper the launcher still ships
  • `a4ee2f58` fix(agent-status): keep owner_waiting lanes in Needs Review
  • `9d8138fb` fix(agent-status): treat idle REPL after complete as settlement, not conflict
  • `6cb5d6e3` fix(agent-status): list receipt files when the git range is empty
