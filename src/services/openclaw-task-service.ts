...
export class OpenClawTaskService implements vscode.Disposable {
	...
	getRecentTasks(limit = 20): OpenClawTask[] {
		return [...this.tasks]
			.sort(
				(left, right) =>
					this.getTaskSortTime(right) - this.getTaskSortTime(left),
			)
			.slice(0, limit);
	}
	...
	async setNotifyPolicy(id: string, policy: string): Promise<void> {
		await this.execCli(["tasks", "notify", id, policy]);
	}
	...
	private parseTasksOutput(stdout: string): OpenClawTask[] {
		...
		return tasks.filter((task) => this.getTaskSortTime(task) >= cutoffMs);
	}
